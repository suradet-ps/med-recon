...
impl Lang {
    ...
    pub fn from_storage() -> Lang {
        if let Some(Ok(Some(storage))) = web_sys::window().map(|w| w.local_storage())
            && let Ok(Some(v)) = storage.get_item("med-recon.lang")
            && v == "en"
        {
            return Lang::English;
        }
    ...
    pub fn save(self) {
        if let Some(Ok(Some(storage))) = web_sys::window().map(|w| w.local_storage()) {
            let _ = storage.set_item("med-recon.lang", self.storage_key());
        }
    }
}
...
pub const KEYS: &[(&str, &str, &str)] = &[
    ...
    (
        ...
        "รหัสผ่านถูกเข้ารหัส ไม่สามารถโหลดกลับมาแสดงได้ — กรอกใหม่ทุกครั้งที่เปิดตั้งค่า",
        "Password is encrypted and cannot be restored into the form — enter it again each time.",
    ),
    ...
    (
        ...
        "Operation timed out (over 25 seconds) — check Host/Port/network and retry.",
    ),
    (
        "settings.note",
    ...
    ("patient.hn", "HN", "HN"),
    ("patient.cid", "CID", "CID"),
    ("patient.birthday", "วันเกิด", "DOB"),
    ("patient.age", "อายุ", "Age"),
    ("patient.age_value", "{n} ปี", "{n}"),
    ...
    ("med.col_no", "ลำดับ", "No."),
    ("med.col_date", "วันที่จ่าย", "Dispense date"),
