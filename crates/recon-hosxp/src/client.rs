...
impl HosxpClient {
    ...
    pub async fn search_patients(&self, query: &str) -> Result<Vec<PatientSummary>> {
        ...
        let rows: Vec<PatientRow> = match recon_core::detect_query_kind(query) {
            recon_core::QueryKind::Cid => {
                self.fetch_rows(
