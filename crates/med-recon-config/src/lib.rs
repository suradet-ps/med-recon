...
/// Keyring service name - shared by all Med Recon users on this machine.
const KEYRING_SERVICE: &str = "MedRecon";
...
/// Platform config directory for the app (`~/Library/Application Support/MedRecon`
/// on macOS, `~/.config/MedRecon` on Linux, `%APPDATA%\MedRecon` on Windows).
fn default_config_dir() -> PathBuf {
    directories::ProjectDirs::from("", "", "MedRecon")
